//src/components/universidad/ModalCrearPerfilEstudiante.tsx
...
function Field({ label, required, hint, children }: { label: string; required?: boolean; hint?: string; children: React.ReactNode }) {
    return (
        <div style={{ display: "flex", flexDirection: "column", gap: "5px" }}>
            ...
            {children}
            {hint && <p style={{ fontSize: "10px", color: "var(--color-text-faint)", margin: 0 }}>{hint}</p>}
        </div>
    );
}
...
/* ══════════════════════════════════════════════════════════════════════
   MODAL CREAR PERFIL ESTUDIANTE
   Acento: color rol universidad (indigo/violet)
══════════════════════════════════════════════════════════════════════ */
export default function ModalCrearPerfilEstudiante({ usuario, onClose, onSuccess }: Props) {
    const [form, setForm] = useState({ codigoUsta: "", programa: "", semestre: "", telefono: "" });
    const [cargando, setCargando] = useState(false);
    const [error, setError] = useState("");
    const [focused, setFocused] = useState("");

    /* rol-universidad tokens */
    const AC = "var(--color-role-universidad)";
    const ABg = "var(--color-role-universidad-bg)";
    const ABr = "rgba(99,102,241,0.25)";
    const AC_HEX = "#6366f1";   /* para el glow/boxShadow que necesita hex */
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const res = await fetch("/api/perfiles/estudiantes", {
            method: "POST",
    ...
    const campos = [
        { name: "codigoUsta", label: "Código USTA", hint: "Número de identificación institucional", placeholder: "Ej: 2180123" },
        { name: "programa", label: "Programa", hint: "Nombre completo del programa académico", placeholder: "Ej: Ingeniería de Telecomunicaciones" },
        { name: "semestre", label: "Semestre", hint: undefined, placeholder: "Ej: 9" },
        { name: "telefono", label: "Teléfono", hint: undefined, placeholder: "Ej: 3001234567" },
    ];

    return (
        <ModalShell onClose={onClose} accentColor={AC} accentBg={ABg} accentBorder={ABr}>
            <ModalHeader icon={<GraduationCap size={15} />} title="Crear perfil de estudiante" subtitle="Datos académicos del practicante" accentColor={AC} accentBg={ABg} accentBorder={ABr} onClose={onClose} />
            ...
            <form onSubmit={handleSubmit} style={{ padding: "20px", display: "flex", flexDirection: "column", gap: "12px" }}>
                {campos.map(c => (
                    <Field key={c.name} label={c.label} hint={c.hint}>
                        <input type="text" name={c.name} value={(form as any)[c.name]} onChange={handleChange}
                            ...
                            style={iStyle(focused === c.name, AC_HEX)} />
                    </Field>
                ))}
                ...
                <div style={{ display: "flex", gap: "10px", paddingTop: "4px" }}>
                    ...
                    <SubmitBtn loading={cargando} label="Crear perfil" loadingLabel="Guardando..." accentColor={AC} accentBg={ABg} icon={<GraduationCap size={14} />} />
                </div>
            </form>
        </ModalShell>
    );
